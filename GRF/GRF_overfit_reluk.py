...
def run():
    ...
    for i in my_indices:
        ...
        for ep in range(500):
            ...
            # save loss curves
        np.savetxt(f'trainloss_overfit_scale_{SCALE}_model_{model_choice}{7-DEPTH}.txt', np.array(train_losses))
        np.savetxt(f'testloss_overfit_scale_{SCALE}_model_{model_choice}{7-DEPTH}.txt', np.array(test_losses))
run()