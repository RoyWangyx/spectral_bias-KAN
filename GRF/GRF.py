...
def run():
    ...
    for i in my_indices:
        ...
        torch.manual_seed(0)
        np.random.seed(0)
        model=MLP(width=WIDTH, depth=DEPTH, dimen=DIM).to(device)
        ...
        for ep in range(500):
            ...
            # save loss curves
        np.savetxt(f'trainloss_dim_{DIM}_scale_{SCALE}_model_{model_choice}.txt', np.array(train_losses))
        np.savetxt(f'testloss_dim_{DIM}_scale_{SCALE}_model_{model_choice}.txt', np.array(test_losses))
run()