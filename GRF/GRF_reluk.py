...
class MLP(nn.Module):
    def __init__(self, width, depth, dimen,k=3):
        super(MLP, self).__init__()
        self.width = width
        if depth < 2:
          raise ValueError(f"depth must be at least 2, got {depth}")
        self.depth = depth
        self.act = ReLUPower(k)
...
def make_dataset(length_scale, input_dim, n_points = 5000):
    torch.manual_seed(0)
    np.random.seed(0)
    # Define parameters
...
def run():
    ...
    for i in my_indices:
        ...
        torch.manual_seed(0)
        np.random.seed(0)
        model=MLP(width=WIDTH, depth=DEPTH, dimen=DIM,k=7-DEPTH).to(device)
        dataset=make_dataset(length_scale=SCALE, input_dim=DIM, n_points = 5000)
        num_train = dataset['train_input'].shape[0]
        num_test = dataset['test_input'].shape[0]
        test_losses = []
        train_losses = []
        for ep in range(500):
            def closure():
                optimizer.zero_grad()
                loss = torch.norm(model(dataset['train_input'])-dataset['train_label'])**2/num_train
            ...
            loss = torch.norm(model(dataset['train_input'])-dataset['train_label'])**2/num_train
            test_loss = torch.norm(model(dataset['test_input'])-dataset['test_label'])**2/(num_test)
            train_losses.append(loss.item())
            test_losses.append(test_loss.item())
            # save loss curves
        np.savetxt(f'trainloss_dim_{DIM}_scale_{SCALE}_model_{model_choice}{7-DEPTH}.txt', np.array(train_losses))
        np.savetxt(f'testloss_dim_{DIM}_scale_{SCALE}_model_{model_choice}{7-DEPTH}.txt', np.array(test_losses))
run()