...
def run():
    ...
    for i in my_indices:
        ...
        for grid in grids[1:]:
            ...
            test_losses.append(result['test_loss'])
            # save loss curves
        np.savetxt(f'trainloss_depth{DEPTH}_dim_{DIM}_scale_{SCALE}_model_{model_choice}.txt', np.array(train_losses).flatten().reshape(-1,1))
        np.savetxt(f'testloss_depth{DEPTH}_dim_{DIM}_scale_{SCALE}_model_{model_choice}.txt', np.array(test_losses).flatten().reshape(-1,1))
run()