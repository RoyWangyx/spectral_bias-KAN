...
# initialize a model; training points are grid-based
        ...
        batchsize = 10000 # batchsize for interior points per direction
        batchsize_b = 1
        update_grid_freq = 5
        ...
        for grid in grids:
            ...
            for ep in range(step):
                ...
                if ep % 10 == 0:
                    ...
                    #plt.show()
        np.savetxt(f'l2pinn{begin_point}_scale_{freq}model_{model_choice}.txt', np.array(true_losses_l2).flatten().reshape(-1,1))
        np.savetxt(f'h1pinn{begin_point}_scale_{freq}model_{model_choice}.txt', np.array(true_losses_h1).flatten().reshape(-1,1))
run()