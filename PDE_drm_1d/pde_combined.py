        ...
        for grid in grids:
            ...
            for ep in range(step):
                ...
                if ep % 10 == 0:
                    ...
                    #plt.show()
        np.savetxt(f'l2_scale_{freq}model_{model_choice}.txt', np.array(true_losses_l2).flatten().reshape(-1,1))
        np.savetxt(f'h1_scale_{freq}model_{model_choice}.txt', np.array(true_losses_h1).flatten().reshape(-1,1))
run()