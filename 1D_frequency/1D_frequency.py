...
def plot_spectral_dynamics(opt, all_frames, para,model_type):
    ...
    plt.title(f'Model{model_type} Width{para[1]} Depth{para[0]} Grid{para[2]}')
    sns.heatmap(norm_dynamics[::-1], 
                xticklabels=opt.K, 
                yticklabels=[(frame.iter_num if frame.iter_num % 1000 == 0 else '') 
    ...
    plt.savefig(f'depth{para[0]}grid{para[2]}model{model_type}_inc.png')
    plt.show()
...
def run():
    ...
    for i in my_indices:
        ...
        eq_amp_frames = go(opt, model_type='KAN')
        plot_spectral_dynamics(opt, eq_amp_frames, params_[i],'KAN')
run()